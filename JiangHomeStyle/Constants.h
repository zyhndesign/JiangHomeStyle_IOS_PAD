//
//  Constants.h
//  JiangHomeStyle
//
//  Created by 工业设计中意（湖南） on 13-9-5.
//  Copyright (c) 2013年 cidesign. All rights reserved.
//

#import <Foundation/Foundation.h>

extern NSString* const INTERNET_VISIT_PREFIX;
extern const int LANDSCAPE_CATEGORY;
extern const int HUMANITY_CATEGORY;
extern const int STORY_CATEGORY;
extern const int COMMUNITY_CATEGORY;

extern const int LANDSCAPE_PAGE_INSIDE_NUM;
extern const int HUMANITY_PAGE_INSIDE_NUM;
extern const int STORY_PAGE_INSIDE_NUM;
extern const int COMMUNITY_INSIDE_NUM;

@interface Constants : NSObject

@end
